...
if(t==NULL)  
{  
t=(struct bstnode *)malloc(sizeof(struct bstnode));  t->data=x;  
t->left=NULL;  
t->right=NULL;  
}  
else if(x<t->data)  
...
struct bstnode *delete1(struct bstnode *t, int x)  {  
 ...
 if(t->left != NULL && t->right != NULL) {   tmp = findmin(t->right);  
 t->data = tmp->data;  
...
10  
1.Insertion, 2.Deletion 3.find, 4.findmin 5.findmax 6.print, 7.exit  enter your choice  
1  
